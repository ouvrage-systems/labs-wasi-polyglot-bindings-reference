...
export class V02TinyLoader extends BaseLoader {
  constructor(paths, scheduler, hostAdapter) {
    super(paths.wasm, hostAdapter);
    this.paths = paths;
  ...
  async instantiateModule() {
    ...
    // Dynamically import the transpiled component ES module based on the component entrypoint path
    const componentUrl = new URL(`../_generated/${this.paths.component_entrypoint_js}`, import.meta.url);
    ...
    if (component.$init) {
      ...
      try {
        ...
        const isCleanExit = e.message?.includes('exit') || e.status === 0 || e.wasiExitCode === 0;
        if (!isCleanExit) {
          throw e;
        }
      }
    ...
    // Get size statistics from the core compiled WASM binary (which is in the same folder as the transpiled JS)
    const coreWasmUrl = new URL(componentUrl.href.replace('.js', '.core.wasm'));
