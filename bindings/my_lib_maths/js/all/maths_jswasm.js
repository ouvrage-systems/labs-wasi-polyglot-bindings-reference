...
export async function jswasmComputeSequence(driver, u0, b, n) {
  let curr = BigInt(u0);
  const step = BigInt(b);
...
export async function jswasmFindLastPrime(driver, limit) {
  let last = 0n;
...
export async function jswasmConcurrentCountPrimes(driver, limit, workers) {
  const max = BigInt(limit);
  if (max < 2n) return 0n;
...
export async function jswasmFibonacci(driver, n) {
  const term = BigInt(n);
...
export async function jswasmFibonacciRecursive(driver, n) {
  const term = BigInt(n);
